...
Memory Controller
    ...
    Takes in (x, y) coordinates from VGA controller to calculate what reel its on, and translate this to RGB color
        Big pixel line, number of times to display that line (fixed number = X, how big a pixel is)
        Once it displays big pixel line X number of times, and repeats this N = 10 times → one sprite is done
        Then, increment sprite counter and restart process 
    Sequentially stops reels in timed order (each following reel does one full rotation before it stops to give it a more randomized looking effect) 
        For each reel store a count N (reel 1 → 5, reel 2 → 6, reel 3 → 7), once the ending symbol index of each reel is fully rendered/displayed N times, stop spinning that reel
...
Symbol ROM Storage
    Stores a contiguous block of equal sized frames for our symbols
    Memory for each reel in terms of what order the symbols should appear in (use this to index into mem)
    FPGA is counting through these for the three reels
SPI Interface Module
    Receives 16 bits commands from MCU, returns an acknowledge signal (ACK) in the LSB
    ...
    MAYBE: display a win/lose message on the display
    REQ == UPDATE: 12 bits contain the total number of points 
LED Display Controller
    Display credits won and total credits on 7 segment displays (NOTE: thinking about getting this on VGA)
